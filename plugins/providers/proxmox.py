...
Requires the [proxmox] optional extra: pip install stratum[proxmox]
...
Credential fields (stored via Stratum integrations UI):
    ...
    token_name      — API token name, e.g. "stratum" (alternative to password)
...
def _get_proxmox(credentials: dict):
    ...
    try:
        ...
        raise RuntimeError("proxmoxer is not installed. Install with: pip install stratum[proxmox]") from exc
...
def execute_build(params: dict) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="stratum-proxmox-") as tmpdir:
        tmp = Path(tmpdir)
        ...
        try:
            ...
            vm_name = f"stratum-build-{profile_name[:20]}-{new_vmid}"
...
def execute_audit(params: dict) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="stratum-proxmox-audit-") as tmpdir:
...
def execute_scan_image(params: dict) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="stratum-proxmox-scan-") as tmpdir:
        tmp = Path(tmpdir)
        key_path, pub_key = utils.generate_ssh_keypair(tmp)
        try:
            new_vmid = _next_vmid(proxmox)
            vm_name = f"stratum-scan-{new_vmid}"
