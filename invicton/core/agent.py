...
"""InvictonBuildAgent — agentic AI build system with pluggable LLM backends.
...
LLM backend is selected by INVICTON_LLM_PROVIDER env var:
  anthropic (default) | openai | ollama | bedrock
See invicton/core/llm/factory.py for full env-var reference.
"""
...
# ---------------------------------------------------------------------------
# Tool implementations — call into Invicton's internal services directly
# ---------------------------------------------------------------------------


def _apply_required_defaults(raw: dict) -> dict:
    """Add invicton_version and kind defaults if missing — they have no schema default."""
    raw.setdefault("invicton_version", "1")
...
async def _start_build(yaml_text: str, provider: str) -> dict:
    ...
    """
    from invicton.core import builder as build_service
    from invicton.core.blueprint import ComplianceProfile
...
TOOLS: list[dict] = [
    {
        ...
        "description": "Validate a blueprint YAML string against the Invicton ComplianceProfile schema. Returns {valid: bool, error?: str, profile_name?, os?, provider?, benchmark?, tier?}",
        "input_schema": {
            "type": "object",
            "properties": {
...
async def _execute_tool(tool_name: str, tool_input: dict) -> Any:
    ...
    if tool_name == "start_build":
        ...
        if settings.invicton_agent_require_confirmation:
            return {
                "error": (
                    "start_build is disabled while INVICTON_AGENT_REQUIRE_CONFIRMATION is enabled "
                    ...
                    "INVICTON_AGENT_REQUIRE_CONFIRMATION=false to let the agent build directly."
                )
            }
...
SYSTEM_PROMPT = """\
You are InvictonBuildAgent, an autonomous OS hardening engineer embedded in the Invicton platform.
...
async def run_build_agent(
    ...
    """Run the InvictonBuildAgent agentic loop.
    ...
    """
    from invicton.core.llm import TextBlock, ToolUseBlock, get_backend
