...
def test_aws_onboarding_templates_have_external_id_default():
    for relpath in (
        "deploy/aws/invicton-scanner-role.yaml",
        "deploy/aws/invicton-builder-role.yaml",
    ):
        ...
        assert {"InvictonRoleArn", "ExternalId", "InstanceProfileName", "RegionHint"} <= set(data["Outputs"])


def test_azure_onboarding_templates_have_required_shape():
    for relpath in (
        "deploy/azure/invicton-scanner-role.json",
        "deploy/azure/invicton-builder-role.json",
    ):
...
def test_gcp_onboarding_scripts_and_roles_have_required_shape():
    ...
    for module in ("scanner", "builder"):
        ...
        role = yaml.safe_load((module_dir / f"invicton-{module}-role.yaml").read_text())
        ...
        assert "invicton-allow-iap-ssh" in script
