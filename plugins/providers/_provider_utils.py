...
"""Shared SSH / Ansible / OpenSCAP utilities for Invicton subprocess providers.
...
def generate_ssh_keypair(tmpdir: Path) -> tuple[Path, str]:
    ...
    """
    key_path = tmpdir / "invicton_id_rsa"
    subprocess.run(
        [
            "ssh-keygen",
            "-t",
            ...
            "",
            "-C",
            "invicton-ephemeral-build",
        ],
        check=True,
        capture_output=True,
    )
    pub = (tmpdir / "invicton_id_rsa.pub").read_text().strip()
...
def run_prehard_ansible_remote(
    ...
    run_remote_cmd(
        ...
        user,
        key_path,
        f"echo '{b64}' | base64 -d | sudo tee /tmp/invicton-prehard.yml > /dev/null",
        port=port,
    )
    ...
    run_remote_cmd(
        ...
        user,
        key_path,
        "sudo ansible-playbook -i 'localhost,' -c local /tmp/invicton-prehard.yml",
        timeout=timeout,
        port=port,
    )
...
def run_hardening_remote(
    ...
    if strategy == "ansible-galaxy":
        ...
        run_remote_cmd(
            ...
            user,
            key_path,
            f"echo '{b64_site}' | base64 -d | sudo tee /tmp/invicton-hardening.yml > /dev/null",
            port=port,
        )
    ...
    cmd = "sudo ansible-playbook -i 'localhost,' -c local /tmp/invicton-hardening.yml"
    if extra_vars:
        ...
        run_remote_cmd(
            ...
            user,
            key_path,
            f"echo '{b64_ev}' | base64 -d | sudo tee /tmp/invicton-extravars.json > /dev/null",
            port=port,
        )
        cmd += " --extra-vars @/tmp/invicton-extravars.json"
...
def install_oscap_on_remote(
    ...
    staging = f"/tmp/invicton-scap-content-{os.path.basename(datastream)}"
...
def run_oscap_remote(
    ...
    profile_id: str,
    datastream: str,
    results_path: str = "/tmp/invicton-oscap.xml",
    timeout: int = 600,
    port: int = 22,
) -> str:
    ...
    cmd = (
        ...
        f"--report /tmp/invicton-oscap-report.html "
        f"{datastream}; "
        f"cat {results_path}"
    )
...
def upload_content_to_remote(
    ...
    try:
        ...
        staging = f"/tmp/invicton-upload-{os.path.basename(remote_path)}"
