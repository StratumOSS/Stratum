...
pip install "stratumoss[all-providers]"
uvicorn stratum.main:app --port 8000
# → http://localhost:8000
```

**First login:** any username + your admin token as the password. If you didn't
set `STRATUM_ADMIN_TOKEN`, a token was generated on first startup and saved to
...
| KVM (local) | none — builds run on the Stratum host itself | nothing; no cloud account required |

The onboarding user must have admin-level permission to create and assign the
required cloud permissions. Stratum shows the full permission set so security
...
means Stratum can authenticate and make a low-risk read call to the provider.
...
Stratum then: provisions a temporary VM → applies the Ansible-Lockdown
