...
This document walks through the complete journey — from a fresh Stratum install to a published, CIS Benchmark Level 2 hardened Amazon Linux 2023 AMI in your AWS account.
...
```yaml
environment:
  STRATUM_SECRET_KEY: "your-strong-passphrase-here"
...
The Stratum home page confirms the engine is running and lists available providers and loaded blueprints.

---

## Step 1 — Add Your AWS Credentials

Stratum mounts `~/.aws` read-only by default. If your credentials are already configured on the host, Stratum picks them up automatically.
...
4. Click **Save** — Stratum encrypts and stores credentials with AES-128 Fernet
...
Stratum offers three entry points. All three produce the same result: a build job driven by a `HardeningBlueprint`.
...
2. On the wizard's first step:
   ...
   - **Cloud Provider:** `AWS`
   - **Region:** `us-east-1`
   - **Base Image:** Stratum resolves the current AL2023 minimal AMI from SSM Parameter Store automatically. The resolved AMI ID is shown in the preview panel.
3. Click **Next**
...
In the UI, go to **Blueprints → Upload** and load the file. Stratum validates the schema on upload and shows any errors inline.
...
2. Type your requirement:
   ...
   Lock root, create stratum-admin user in wheel group.
...
```
[Stratum AI] Generating HardeningBlueprint for Amazon Linux 2023 CIS L2...
[Stratum AI] Blueprint validated. Selecting base image: ami-0230bd60aa48260c6 (us-east-1).
[Stratum AI] Launching build pipeline...
```
...
- Stratum calls the AWS provider to create a t3.medium EC2 instance from the base AMI
...
Stratum generates a two-phase Ansible run:
...
- Create the `stratum-admin` user account
...
Stratum runs OpenSCAP against the hardened instance:
...
- Stratum creates an AMI from the hardened instance
- Tags it with compliance metadata:
  ```
  stratum:profile     = cis_server_l2
  stratum:os          = amazon-linux-2023
  stratum:grade       = A
  stratum:score       = 94.6
  stratum:built-by    = stratum
  stratum:blueprint   = amzn2023-cis-l2-aws
  stratum:build-date  = 2026-04-21
  ```
  ...
  image-build:
    ...
    steps:
      ...
      - name: Trigger Stratum build
        ...
        run: |
          RESP=$(curl -sf \
            -H "X-API-Key: ${{ secrets.STRATUM_API_KEY }}" \
            -H "Content-Type: application/json" \
            -d @profiles/user/amazon-linux-2023-cis-l2-aws.yaml \
            "${{ vars.STRATUM_URL }}/api/builder/build")
      ...
      - name: Wait for build to complete
        run: |
          ...
          for i in $(seq 1 40); do
            STATUS=$(curl -sf \
              -H "X-API-Key: ${{ secrets.STRATUM_API_KEY }}" \
              "${{ vars.STRATUM_URL }}/api/builder/build/$JOB_ID" \
              | jq -r '.status')
      ...
      - name: Gate on compliance grade
        run: |
          RESULT=$(curl -sf \
            -H "X-API-Key: ${{ secrets.STRATUM_API_KEY }}" \
            "${{ vars.STRATUM_URL }}/api/builder/build/${{ steps.build.outputs.job_id }}")
...
Add a webhook in **Settings → Webhooks** to receive a notification when drift is detected (any rule regresses from pass to fail). Stratum signs all webhook payloads with HMAC-SHA256.
