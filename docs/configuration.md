...
|---|---|---|
| `INVICTON_ADMIN_TOKEN` | auto-generated | Login for the UI and API (HTTP Basic — any username, this as the password). If unset, a random token is generated on first startup and saved to `data/.admin_token`; check there (or the container logs) to log in. Set it explicitly for a stable login. |
| `INVICTON_SECRET_KEY` | auto-generated | Passphrase for the credential-encryption key (Fernet). If unset, a random key is stored in `data/.invicton_key`. Set it so onboarded cloud credentials survive container rebuilds. |
| `INVICTON_AGENT_REQUIRE_CONFIRMATION` | `true` | Require a human click before the AI Builder provisions real cloud infrastructure. |
...
Pick one via `INVICTON_LLM_PROVIDER`:

| Provider | `INVICTON_LLM_PROVIDER` | Auth | Notes |
|---|---|---|---|
| Anthropic | `anthropic` | `ANTHROPIC_API_KEY` | Default; extended thinking on by default |
| OpenAI / compatible | `openai` | `INVICTON_LLM_API_KEY` | Groq, Together, vLLM, LiteLLM, Fireworks |
...
```bash
INVICTON_LLM_PROVIDER=anthropic   # anthropic | openai | ollama | bedrock
INVICTON_LLM_MODEL=               # override the backend's default model
INVICTON_LLM_API_KEY=             # for openai-compatible backends
INVICTON_LLM_BASE_URL=            # any OpenAI-compatible endpoint (Ollama: http://localhost:11434/v1)
INVICTON_LLM_THINKING=1           # extended thinking (Anthropic/Bedrock), 1=on 0=off
```
...
Invicton aggregates blueprints from the community GitHub library, an optional
...
BLUEPRINT_STORE_S3_PREFIX=invicton/
BLUEPRINT_STORE_S3_REGION=us-east-1
```

## System dependencies for real builds

Invicton itself is pure Python, but hardening and scanning shell out to
...
- Ubuntu 22.04 has no `openscap-scanner` apt package in any channel (it first
  appears in 24.04) — scanning targets running 22.04 relies on Invicton's
