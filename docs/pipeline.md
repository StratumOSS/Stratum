# Invicton — Pipeline Integration Guide

Use Invicton as a compliance gate in any CI/CD pipeline. After building a hardened image, scan it automatically and fail the pipeline if it doesn't meet your thresholds.

---

## Prerequisites

1. Invicton is running and reachable from your CI environment (e.g. `http://invicton:8001`)
...
Generate a key from the Invicton UI or via the API:

```bash
curl -s -X POST http://invicton:8001/api/api-keys \
...
```bash
curl -sf -X POST http://invicton:8001/api/pipeline/scan \
...
```json
{
  ...
  "error":                null,
  "report_url":           "http://invicton:8001/api/auditor/scan-image/.../report?fmt=json",
  "sarif_url":            "http://invicton:8001/api/auditor/scan-image/.../report?fmt=sarif",
  "html_report_url":      "http://invicton:8001/api/auditor/scan-image/.../report"
}
```
...
Invicton can build the hardened image itself — provision, harden, scan, snapshot — and return the artifact ID directly to your pipeline.

### Quick Build Trigger

```bash
curl -sf -X POST http://invicton:8001/api/pipeline/build \
  -H "X-API-Key: $INVICTON_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
INVICTON_URL="${INVICTON_URL:-http://invicton:8001}"
INVICTON_API_KEY="${INVICTON_API_KEY:?Set INVICTON_API_KEY}"
PROFILE="${1:-ubuntu22-cis-l1-aws}"

echo "Building $PROFILE ..."
BUILD=$(curl -sf -X POST "$INVICTON_URL/api/pipeline/build" \
  -H "X-API-Key: $INVICTON_API_KEY" \
  -H "Content-Type: application/json" \
...
curl -sf -X POST "$INVICTON_URL/api/pipeline/scan" \
  -H "X-API-Key: $INVICTON_API_KEY" \
  -H "Content-Type: application/json" \
  -d "{
    \"image_id\":           \"$ARTIFACT\",
  ...
  build-and-gate:
    ...
    steps:
      - name: Trigger Invicton build
        id: build
        env:
          INVICTON_API_KEY: ${{ secrets.INVICTON_API_KEY }}
        run: |
          BUILD=$(curl -sf -X POST ${{ vars.INVICTON_URL }}/api/pipeline/build \
            -H "X-API-Key: $INVICTON_API_KEY" \
            -H "Content-Type: application/json" \
      ...
      - name: Compliance gate
        ...
        run: |
          curl -sf -X POST ${{ vars.INVICTON_URL }}/api/pipeline/scan \
            -H "X-API-Key: $INVICTON_API_KEY" \
            -H "Content-Type: application/json" \
            -d "{
...
```bash
JOB_ID=$(curl -sf -X POST "$INVICTON_URL/api/pipeline/build" \
  -H "X-API-Key: $INVICTON_API_KEY" \
  -H "Content-Type: application/json" \
...
while true; do
  RESP=$(curl -sf "$INVICTON_URL/api/pipeline/build/$JOB_ID" \
    -H "X-API-Key: $INVICTON_API_KEY")
  ...
  compliance-gate:
    ...
    steps:
      - name: Compliance Scan
        ...
        run: |
          RESULT=$(curl -sf -X POST ${{ vars.INVICTON_URL }}/api/pipeline/scan \
            -H "X-API-Key: $INVICTON_API_KEY" \
            -H "Content-Type: application/json" \
            -d "{
              \"image_id\":           \"$AMI_ID\",
          ...
          curl -sf "$SARIF_URL" -H "X-API-Key: $INVICTON_API_KEY" > invicton.sarif.json
      ...
      - name: Upload SARIF
        ...
        with:
          sarif_file: invicton.sarif.json
          category: invicton-compliance
```
...
compliance-scan:
  ...
  script:
    - |
      RESULT=$(curl -sf -X POST "$INVICTON_URL/api/pipeline/scan" \
        -H "X-API-Key: $INVICTON_API_KEY" \
        -H "Content-Type: application/json" \
        -d "{
          \"image_id\":           \"$AMI_ID\",
    ...
    stage('Compliance Gate') {
      steps {
        script {
          def response = sh(
            script: """
              curl -sf -X POST ${env.INVICTON_URL}/api/pipeline/scan \\
                -H "X-API-Key: ${env.INVICTON_API_KEY}" \\
                -H "Content-Type: application/json" \\
...
JOB_ID=$(curl -sf -X POST "$INVICTON_URL/api/pipeline/scan" \
  -H "X-API-Key: $INVICTON_API_KEY" \
  -H "Content-Type: application/json" \
...
# Poll until complete
while true; do
  STATUS=$(curl -sf "$INVICTON_URL/api/pipeline/scan/$JOB_ID" \
    -H "X-API-Key: $INVICTON_API_KEY" | jq -r '.status')
  echo "Status: $STATUS"
  [ "$STATUS" = "complete" ] || [ "$STATUS" = "failed" ] && break
  sleep 30
done
...
# Get final result
curl -sf "$INVICTON_URL/api/pipeline/scan/$JOB_ID" \
  -H "X-API-Key: $INVICTON_API_KEY" | jq -e '.passed == true'
```
...
```bash
# Register
curl -X POST "$INVICTON_URL/api/webhooks" \
  -H "Content-Type: application/json" \
  -d '{
...
curl -X POST \
  ...
  --data @invicton.sarif.json \
  "https://api.github.com/repos/$OWNER/$REPO/code-scanning/sarifs"
```

**Azure DevOps:**

Use the [PublishBuildArtifacts task](https://learn.microsoft.com/en-us/azure/devops/pipelines/tasks/reference/publish-build-artifacts-v1) to upload `invicton.sarif.json`, then view it in the Security tab.
...
```bash
curl -X POST "$INVICTON_URL/api/pipeline/verify/$JOB_ID?pass_threshold=90&severity_threshold=critical" \
  -H "X-API-Key: $INVICTON_API_KEY" \
...
```bash
curl -sf -X POST http://invicton:8001/api/blueprints/upload \
...
```bash
curl -sf -X POST http://invicton:8001/api/blueprints/validate \
...
```yaml
- id: invicton-blueprint-validate
  name: Validate Invicton blueprint
  language: system
  entry: bash -c 'curl -sf -X POST http://invicton:8001/api/blueprints/validate
...
```bash
curl -sf -X DELETE http://invicton:8001/api/blueprints/ubuntu22-cis-l1-aws
# → 204 No Content
```
...
```markdown
![Compliance](http://invicton:8001/api/auditor/scan-image/{job_id}/badge.svg)
```
