...
class TestReportGaps:
    ...
    def test_delta_report_none_scores_trend_stable(self):
        ...
        from invicton.core.report import generate_delta_report

        none_scan = {"score": None, "rules": []}
        with patch("invicton.core.report.parse_arf", return_value=none_scan):
    ...
    def test_delta_report_zero_baseline_score_pct_change_zero(self):
        ...
        with patch("invicton.core.report.parse_arf", side_effect=_fake_parse):
            result = generate_delta_report(Path("/fake/a.xml"), Path("/fake/b.xml"))
        assert result["pct_change"] == 0.0
    ...
    def test_register_formatter_non_subclass_raises_type_error(self):
        ...
        from invicton.core.report import register_formatter
    ...
    def test_generate_summary_with_mocked_parse_arf(self):
        ...
        from invicton.core.report import generate_summary
        ...
        with patch("invicton.core.report.parse_arf", return_value=fake_data):
...
class TestOpenSCAPParserGaps:
    ...
    def test_parse_arf_no_test_result_raises(self, tmp_path):
        ...
        from invicton.openscap.parser import parse_arf
...
"""
    ...
    def test_enrich_blueprint_validation_failure_returns_error(self):
        ...
        from invicton.core.agent import _enrich_blueprint
    ...
    def test_retry_build_yaml_invalid_yaml_returns_fallback(self):
        ...
        from invicton.core.agent import _retry_build_yaml
    ...
    async def test_execute_tool_start_build(self):
        ...
        from invicton.core.agent import _execute_tool
        ...
        with patch("invicton.core.builder.run_build", new_callable=AsyncMock):
    ...
    async def test_execute_tool_get_scan_report(self):
        ...
        with patch("invicton.core.builder.get_job", return_value=None):
    ...
    async def test_run_build_agent_agent_turn_exception_returns_result_with_error(self):
        ...
        from invicton.core.agent import run_build_agent
        from invicton.core.llm.base import LLMBackend
        ...
        with patch("invicton.core.llm.get_backend", return_value=FailingBackend()):
    ...
    async def test_run_build_agent_tool_call_emits_on_token(self):
        ...
        from invicton.core.agent import run_build_agent
        from invicton.core.llm.base import AgentTurnResult, TextBlock, ToolUseBlock
        ...
        with patch("invicton.core.llm.get_backend", return_value=MockBackend()):
...
class TestAuditorWebhookExcept:
    ...
    async def test_run_audit_webhook_create_task_raises(self):
        ...
        from invicton.core.auditor import AuditStatus, run_audit
        ...
        with patch("invicton.core.auditor._persist_jobs"):
...
class TestAuditorRunImageScanException:
    ...
    async def test_run_image_scan_exception_sets_job_failed(self):
        ...
        from invicton.core.auditor import AuditStatus, run_image_scan
        from invicton.core.blueprint import ComplianceProfile
        ...
        with patch("invicton.core.auditor.registry") as mock_reg:
